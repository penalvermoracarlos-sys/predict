...
const PredictionLog = require("../models/PredictionLog");   // <-- Import MongoDB model
...
async function doPredict(req, res) {
  ...
  try {
    ...
    console.log(">>> EJECUTANDO doPredict CON MONGO");


    // Respuesta al cliente con ID real
    res.status(201).json({
      predictionId: log._id,
      prediction,
      timestamp,
      latencyMs
    });
    ...
    console.error("💥 ERROR REAL EN /predict:");
    console.error(err);
